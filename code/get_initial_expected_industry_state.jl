...
function get_initial_expected_industry_state!(
    df::DataFrame,
    state_space::String,
    x::DataFrame,
    Ω_df::DataFrame,
    ...
    A_df::DataFrame,
    d::DG_data
    )::Array{Float64,2} #TODO: verify the type of the output

    df.Ω_index = findnearest(Ω_df.Ω, df.Ω) # describe(df.Ω_index)
    df.K_index = findnearest(K_df.K, df.K) # describe(df.K_index)
    df.A_index = findnearest(A_df.A, df.A) # describe(df.A_index)
    if state_space == "KAΩ"
        s_df = df[:,[:K_index,:Ω_index,:A_index, :tid]]
